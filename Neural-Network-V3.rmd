...
newdat=read.csv(file="project2_doeresults_v3.csv")
library(ISLR)
library(glmnet)
set.seed(1)
attach(newdat)
...
```{r}
library(neuralnet)
BPNN3=neuralnet(status~dep+len+dia+t+mat+flow+velo,data=train,hidden=c(5,3),linear.output = FALSE, threshold = 0.05,stepmax = 1e+06)
plot(BPNN3,rep="best")
```
